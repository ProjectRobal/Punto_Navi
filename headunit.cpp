...
int Headunit::startHU(){
    QSettings settings("aa_settings.ini",QSettings::IniFormat);
    ...
    if(settings.childGroups().contains("aa_settings")){
        ...
        for (int i = 0; i < keys.size(); ++i){
            ...
            qDebug()<<aa_settings[keys.at(i).toStdString()].c_str();
        }
    }
  ...
  /*  if(settings.childKeys().contains("resolution")){
    ...
    else if(error_counter<max_attempts)
    {
        error_counter++;

        qDebug()<<"Failed to connect trying again for "<<error_counter<<"(of "<<max_attempts<<") time";
...
int Headunit::init(QWidget * videoItem){

    GstBus *bus;

    GError *error = NULL;

        qDebug()<<"initialize video pipeline";
   ...
   // gst_object_unref(vid_pipeline);
    ...
    //GstElement *app_src=gst_element_factory_make("video_src",NULL);
    //GstElement vid_sink=gst_bin_get_by_name(*GST_BIN(*vid_pipeline), "vid_sink");

    //gst_element_link_pads (vid_sink, "src", app_src, "sink");

   // player->setVideoOutput(videoItem);

   // player->setMedia(QUrl("gst-pipeline: appsrc name=video_sink ! xvimagesink name=qtvideosink"));

    GstVideoOverlay* sink = GST_VIDEO_OVERLAY(gst_bin_get_by_name(GST_BIN(vid_pipeline), "vid_sink"));

    WId window=videoItem->winId();

        gst_video_overlay_set_window_handle(sink, window);
...
void Headunit::touchEvent(HU::TouchInfo::TOUCH_ACTION action, QPoint *point) {
    ...
    if(huStarted){
     //   qDebug()<<m_outputWidth<<" "<<m_outputHeight;
...
void Headunit::setOutputSize(const QSize _size)
{

    m_outputWidth=_size.width();
    m_outputHeight=_size.height();
